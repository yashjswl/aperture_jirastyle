  ...
  return (
    <div className="space-y-6 animate-slide-up">
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        {/* Left Column: Engagement & Assignments */}
        <div className="lg:col-span-2 space-y-6">
          {user.role === "WORKING_TEAM" && engagement && (
            <Card className="bg-surface border-white/5 overflow-hidden flex flex-col">
              ...
              <div className="divide-y divide-white/5">
                {myAssignedCalls.length === 0 ? (
                  ...
                  myAssignedCalls.map(({ event }) => (
                    <Link key={event.id} href={`/coverages/${event.id}`} className="flex items-center justify-between p-3 hover:bg-white/5 transition-colors group">
                      <div className="flex items-center gap-3">
                        <span className="text-[11px] font-mono text-white/30 uppercase">#{event.id.slice(10, 15)}</span>
                        <p className="text-sm font-medium text-white/90 group-hover:text-accent transition-colors">{event.title}</p>
                      </div>
                      <div className="flex items-center gap-3">
                        {event.location && <span className="text-[10px] font-bold uppercase text-white/40 bg-white/5 px-2 py-0.5 rounded">{event.location}</span>}
          ...
          {showMembers && (
            <div className="grid grid-cols-2 gap-6">
              <Card className="bg-surface border-white/5 p-4 flex items-center justify-between">
                <div>
                  <p className="text-[11px] text-white/40 uppercase font-bold tracking-wider mb-1">Active Members</p>
                  <p className="text-2xl font-bold tracking-tight text-white/90">{memberCount}</p>
                </div>
              ...
              <Card className="bg-surface border-white/5 p-4 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm font-medium text-white/90 mt-2">Check Schedule</p>
                </div>
